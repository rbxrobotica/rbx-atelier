...
- **Logo / wordmark.** The **official Strategos mark** lives in `assets/strategos-mark.svg`: a customized lowercase Greek sigma with a hawk-beak terminal, single `path`, `fill="currentColor"`, square `viewBox 0 0 1000 1000`. Supplied by the founder on 2026-08-20 (drafted with ChatGPT, Claude, and Claude Design), replacing the earlier placeholder "S in a chamfered square". Render it in `--ink-0`; per rule 6, any accent on brand chrome is Cyan, and the mark is never tinted Aurum. `currentColor` only inherits when the SVG is inlined or rendered through a component; `<img>`/favicon embeds do not inherit `color`, so use `assets/strategos-mark-adaptive.svg` (explicit theme-adaptive fill) for those. Canonical usage doc: `BRAND-MARK.md` in this folder; reference implementations: `strategos-site/src/components/StrategosMark.tsx` (React) and `strategos-ui/src/lib/components/LogoStrategos.svelte` (Svelte).
